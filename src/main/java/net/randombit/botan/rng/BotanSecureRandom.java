 ...
 *
 * <p>This class provides cryptographically secure pseudo-random number generation (CSPRNG) using
 * the Botan cryptographic library's native random number generators. It integrates seamlessly with
 * Java's {@link java.security.SecureRandom} API while leveraging Botan's randomness API.
 *
 * <h2>Supported RNG Types</h2>
 *
 * <p>Botanj supports multiple RNG types through Botan:
 *
 * <ul>
 *   <li><b>system</b> - System RNG (default): Uses OS-provided entropy sources (e.g., /dev/urandom
 *       on Unix, CryptGenRandom on Windows)
 *   <li><b>user</b> - User-space RNG: ChaCha20-based CSPRNG seeded from system RNG
 *   <li><b>user-threadsafe</b> - Thread-safe user-space RNG: Same as user but with internal locking
 ...
 * <h2>Thread Safety</h2>
 *
 * <p>Thread safety depends on the RNG type:
 *
 * <ul>
 *   <li><b>system RNG</b>: Thread-safe (backed by OS)
 *   <li><b>user RNG</b>: NOT thread-safe - use separate instances per thread
 *   <li><b>user-threadsafe RNG</b>: Thread-safe with internal locking
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>RNG objects are destroyed when the Java object is garbage collected
 *   <li>Cleaner ensures cleanup even if exceptions occur
 * </ul>
 *
 * <h2>Security Considerations</h2>
 *
 * <ul>
 *   <li><b>Cryptographically Secure</b>: All RNG types are suitable for cryptographic use
 *   <li><b>Automatic Seeding</b>: System RNG automatically seeds user-space RNGs
 *   <li><b>Entropy Addition</b>: {@link #engineSetSeed(byte[])} adds entropy without replacing
 *       existing state
 *   <li><b>No Prediction</b>: Internal state cannot be predicted from outputs
 * </ul>
 *
 * @author Yasser Aziza
...
public abstract class BotanSecureRandom extends SecureRandomSpi {
   ...
   *
   * <p>Uses the operating system's entropy source directly. This is the most secure option as it
   * bypasses any userspace buffering and gets randomness directly from the OS.
   *
   * <p>Thread-safe: Yes (backed by OS)
   */
   ...
   *
   * <p>ChaCha20-based CSPRNG that is automatically seeded from the system RNG. Faster than system
   * RNG for generating large amounts of random data, while maintaining cryptographic security.
   *
   * <p>Thread-safe: No (use separate instances per thread or use UserThreadsafeRng)
   */
   ...
   *
   * <p>Same as UserRng but with internal locking to allow safe concurrent access from multiple
   * threads. Slight performance overhead compared to UserRng due to synchronization.
   *
   * <p>Thread-safe: Yes
   */
