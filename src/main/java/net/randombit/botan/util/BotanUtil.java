...
public final class BotanUtil {
   ...
   */
  public static void checkKeySize(
      Pointer ptr, int keySize, FourParameterFunction<Pointer, NativeLongByReference> getKeySpec)
    ...
    if (keySize < minimumLength.intValue()) {
      throw new InvalidKeyException(
          "key.getEncoded() < minimum key length: " + minimumLength.intValue());
    }

    if (keySize > maximumLength.intValue()) {
      throw new InvalidKeyException(
          "key.getEncoded() > maximum key length: " + maximumLength.intValue());
    }

    if (keySize % lengthModulo.intValue() != 0) {
      throw new InvalidKeyException(
          "key.getEncoded() not multiple of key length modulo: " + lengthModulo.intValue());
    }
  }
