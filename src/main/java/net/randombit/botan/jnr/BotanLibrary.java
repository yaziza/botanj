 ...
 *
 * <p>This interface provides direct access to Botan's C FFI (Foreign Function Interface) through
 * JNR-FFI. It defines Java method signatures that map to native Botan functions, handling type
 * conversions and memory management for cross-language calls.
 *
 ...
 *
 * <ul>
 *   <li><b>Interface, not Implementation:</b> This is a JNR-FFI interface - JNR automatically
 *       generates the implementation by binding to the native library at runtime
 *   <li><b>Direct Native Access:</b> All methods directly correspond to Botan C FFI functions
 *   <li><b>Memory Safety:</b> Uses JNR-FFI annotations (@In, @Out) to manage native memory safely
 *   <li><b>Singleton Access:</b> Instances are created and managed by {@link BotanInstance}
 * </ul>
 *
 * <h2>Function Categories</h2>
 *
 * <p>The library provides bindings for the following Botan operations:
 *
 * <h3>Utility Functions</h3>
 *
 * <ul>
 *   <li>{@link #botan_version_string()} - Get Botan library version
 *   <li>{@link #botan_ffi_supports_api(long)} - Check FFI API version support
 *   <li>{@link #botan_error_description(int)} - Convert error codes to descriptions
 * </ul>
 *
 * <h3>Encoding Functions</h3>
 *
 * <ul>
 *   <li>{@link #botan_hex_encode(byte[], long, byte[], long)} - Binary to hexadecimal
 *   <li>{@link #botan_hex_decode(byte[], long, byte[], NativeLongByReference)} - Hexadecimal to
 *       binary
 *   <li>{@link #botan_base64_encode(byte[], long, byte[], NativeLongByReference)} - Binary to
 *       Base64
 *   <li>{@link #botan_base64_decode(String, long, byte[], NativeLongByReference)} - Base64 to
 *       binary
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>{@link #botan_hash_init(PointerByReference, String, long)} - Initialize hash object
 *   <li>{@link #botan_hash_update(Pointer, byte[], long)} - Add data to hash
 *   <li>{@link #botan_hash_final(Pointer, byte[])} - Finalize and get digest
 *   <li>{@link #botan_hash_copy_state(PointerByReference, Pointer)} - Clone hash state
 *   <li>{@link #botan_hash_clear(Pointer)} - Reset hash state
 *   <li>{@link #botan_hash_destroy(Pointer)} - Free hash object
 * </ul>
 *
 * <h3>Message Authentication Codes (MACs)</h3>
 *
 * <ul>
 *   <li>{@link #botan_mac_init(PointerByReference, String, long)} - Initialize MAC object
 *   <li>{@link #botan_mac_set_key(Pointer, byte[], long)} - Set MAC key
 *   <li>{@link #botan_mac_update(Pointer, byte[], long)} - Add data to MAC
 *   <li>{@link #botan_mac_final(Pointer, byte[])} - Finalize and get MAC tag
 *   <li>{@link #botan_mac_clear(Pointer)} - Reset MAC state
 *   <li>{@link #botan_mac_destroy(Pointer)} - Free MAC object
 * </ul>
 *
 * <h3>Symmetric Ciphers</h3>
 *
 * <ul>
 *   <li>{@link #botan_cipher_init(PointerByReference, String, long)} - Initialize cipher object
 *   <li>{@link #botan_cipher_set_key(Pointer, byte[], long)} - Set cipher key
 *   <li>{@link #botan_cipher_start(Pointer, byte[], long)} - Start cipher with IV/nonce
 *   <li>{@link #botan_cipher_update(Pointer, long, byte[], long, NativeLongByReference, byte[],
 *       long, NativeLongByReference)} - Process data (encrypt/decrypt)
 *   <li>{@link #botan_cipher_reset(Pointer)} - Reset cipher state
 *   <li>{@link #botan_cipher_destroy(Pointer)} - Free cipher object
 * </ul>
 *
 * <h3>Random Number Generators (RNG)</h3>
 *
 * <ul>
 *   <li>{@link #botan_rng_init(PointerByReference, String)} - Initialize RNG object
 *   <li>{@link #botan_rng_get(Pointer, byte[], long)} - Generate random bytes
 *   <li>{@link #botan_rng_reseed(Pointer, long)} - Reseed RNG from system entropy
 *   <li>{@link #botan_rng_reseed_from_rng(Pointer, Pointer, long)} - Reseed from another RNG
 *   <li>{@link #botan_rng_add_entropy(Pointer, byte[], long)} - Add entropy to RNG
 *   <li>{@link #botan_rng_destroy(Pointer)} - Free RNG object
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>0</b> indicates success
 *   <li><b>Negative values</b> indicate errors (e.g., -1 for invalid input, -20 for bad MAC)
 *   <li>Use {@link #botan_error_description(int)} to convert error codes to human-readable messages
 *   <li>{@link BotanInstance#checkNativeCall(int, String)} provides centralized error checking
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>Initialization functions</b> (e.g., botan_hash_init) allocate native memory
 *   <li><b>Destroy functions</b> (e.g., botan_hash_destroy) must be called to free memory
 *   <li><b>Java wrappers</b> use Cleaner API to ensure automatic cleanup on garbage collection
 *   <li><b>PointerByReference</b> is used for output parameters that receive native object handles
 * </ul>
 *
 * <h2>Usage Pattern</h2>
 *
 * <p>Direct usage of this interface is not recommended. Instead, use the high-level JCA wrapper
 * classes:
 *
 * <pre>{@code
 ...
 *
 * <ul>
 *   <li><b>@In</b> - Input parameter (Java to native)
 *   <li><b>@Out</b> - Output parameter (native to Java)
 *   <li><b>PointerByReference</b> - Reference to a native pointer (for object handles)
 *   <li><b>NativeLongByReference</b> - Reference to a native long (for output sizes)
 *   <li><b>Pointer</b> - Opaque native pointer (for object handles)
 * </ul>
 *
 * <h2>Thread Safety</h2>
 *
 ...
 *
 * <ul>
 *   <li>Each native object (hash, MAC, cipher) must be accessed by only one thread
 *   <li>Different threads can use different native objects concurrently
 *   <li>Java wrapper classes inherit this thread-safety model
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>Library name:</b> "botan-3"
 *   <li><b>Minimum version:</b> Botan 3.0.0
 *   <li><b>FFI API:</b> Uses Botan's C FFI (stable ABI)
 *   <li><b>Platform support:</b> Linux, macOS, Windows
 * </ul>
 *
 * <h2>Implementation Notes</h2>
 *
 * <ul>
 *   <li><b>Auto-generated Implementation:</b> JNR-FFI generates the implementation at runtime by
 *       analyzing this interface and creating native bindings
 *   <li><b>No Manual JNI:</b> No need for hand-written JNI code - JNR handles everything
 *   <li><b>Type Mapping:</b> JNR automatically maps Java types to C types (int→int,
 *       byte[]→uint8_t*, etc.)
 *   <li><b>Performance:</b> Near-native performance due to efficient JNR code generation
 * </ul>
 *
 * @author Yasser Aziza
 * @see BotanInstance
...
public interface BotanLibrary {
  ...
  /**
   * Converts an error code into a string. Returns "Unknown error" if the error code is not a known
   * one.
   *
  ...
  /**
   * Returns the version of the currently supported FFI API. This is expressed in the form YYYYMMDD
   * of the release date of this version of the API.
   *
   ...
   * @param flags should be 0 in current API revision, all other uses are reserved and return
  ...
  /**
   * Finalizes the hash computation and writes the output to out[0:botan_hash_output_length()] then
   * reinitializes for computing another digest as if botan_hash_clear had been called.
   *
   * @param hash hash object
  ...
  /**
   * Reinitializes the state of the hash computation. A hash can be computed (with update/final)
   * immediately.
   *
   * @param hash hash object
   * @return 0 on success, a negative value on failure
   ...
   * @param flags should be 0 in current API revision, all other uses are reserved and return a
  ...
  /**
   * Finalizes the MAC computation and writes the output to out[0:botan_mac_output_length()] then
   * reinitializes for computing another MAC as if botan_mac_clear had been called.
   *
   * @param mac MAC object
   * @param out output buffer
   * @return 0 on success, a negative value on failure
  ...
  /**
   * Reinitializes the state of the MAC computation. A MAC can be computed (with update/final)
   * immediately.
   *
   * @param mac MAC object
   * @return 0 on success, a negative value on failure
   ...
   * @return 0 on success, a negative value on failure
   */
  int botan_mac_get_keyspec(
      @In Pointer mac,
      @Out NativeLongByReference minimumKeyLength,
   ...
   * @return 0 on success, a negative value on failure
   */
  int botan_cipher_output_length(
      @In Pointer cipher, @In long inputLength, @In @Out NativeLongByReference outputLength);
   ...
   * @return 0 on success, a negative value on failure
   */
  int botan_cipher_get_default_nonce_length(
      @In Pointer cipher, @Out NativeLongByReference nonceLength);

  /**
   * Returns the update granularity of the cipher; botan_cipher_update must be called with blocks of
   * this size, except for the final.
   *
   * @param cipher cipher object
   * @param updateGranularity update granularity
   * @return 0 on success, a negative value on failure
   */
  int botan_cipher_get_update_granularity(
      @In Pointer cipher, @Out NativeLongByReference updateGranularity);
   ...
   * @return 0 on success, a negative value on failure
   */
  int botan_cipher_get_keyspec(
      @In Pointer cipher,
      @Out NativeLongByReference minKeylen,
      @Out NativeLongByReference maxKeylen,
      @Out NativeLongByReference modKeylen);
  ...
  /**
   * Resets the message specific state for this cipher. Without resetting the keys, this resets the
   * nonce, and any state associated with any message bits that have been processed so far. It is
   * conceptually equivalent to calling botan_cipher_clear followed by botan_cipher_set_key with the
   * original key.
   *
   * @param cipher cipher object
   * @return 0 on success, a negative value on failure
