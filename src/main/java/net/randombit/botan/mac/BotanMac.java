...
/**
 * Abstract base class for Message Authentication Code (MAC) implementations using the Botan
 * cryptography library.
 *
 * <p>This class provides a JCE-compliant MAC implementation that delegates cryptographic operations
 * to native Botan library functions via JNR-FFI. It implements automatic native resource management
 * using the Java {@link Cleaner} API to ensure native MAC objects are properly destroyed when no
 * longer needed.
 *
 ...
 * <h2>Thread Safety</h2>
 *
 * <p>This implementation is NOT thread-safe. Each thread should use its own MAC instance. The JCE
 * API does not require MAC implementations to be thread-safe.
 *
 ...
 *
 * <ul>
 *   <li>{@code macFinalized} - tracks whether {@code doFinal()} has been called, which auto-resets
 *       the state
 *   <li>{@code currentKey} - stores the current key for re-initialization after explicit {@code
 *       reset()}
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li><b>CMAC</b> - {@link CMac} - Cipher-based MAC using AES (16-byte output)
 *   <li><b>Poly1305</b> - {@link Poly1305} - Poly1305 MAC (16-byte output)
 *   <li><b>SipHash</b> - {@link SipHash} - SipHash-2-4 (8-byte output)
 *   <li><b>HMAC-MD5</b> - {@link HMacMd5} - HMAC with MD5 (16-byte output)
 *   <li><b>HMAC-RIPEMD160</b> - {@link HMacRipeMd160} - HMAC with RIPEMD-160 (20-byte output)
 *   <li><b>HMAC-SHA1</b> - {@link HMacSha1} - HMAC with SHA-1 (20-byte output)
 *   <li><b>HMAC-SHA224</b> - {@link HMacSha224} - HMAC with SHA-224 (28-byte output)
 *   <li><b>HMAC-SHA256</b> - {@link HMacSha256} - HMAC with SHA-256 (32-byte output)
 *   <li><b>HMAC-SHA384</b> - {@link HMacSha384} - HMAC with SHA-384 (48-byte output)
 *   <li><b>HMAC-SHA512</b> - {@link HMacSha512} - HMAC with SHA-512 (64-byte output)
 * </ul>
 *
 ...
 *       CloneNotSupportedException} because native MAC state cannot be safely cloned
 *   <li><b>Key Size Validation</b> - Key sizes are validated against Botan's key specification
 *       during initialization
 *   <li><b>Auto-Reset Optimization</b> - After {@code doFinal()}, the MAC is automatically reset by
