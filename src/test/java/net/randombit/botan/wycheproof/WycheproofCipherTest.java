 ...
 *
 * <p>This class provides infrastructure for testing symmetric ciphers using
 * Wycheproof test vectors. These tests focus on IndCPA (Indistinguishability
 * under Chosen Plaintext Attack) properties of encryption schemes.</p>
 */
public abstract class WycheproofCipherTest extends WycheproofTestBase {
     ...
     */
    protected void runWycheproofCipherTests(String resourcePath, String transformation) throws Exception {
                ...
                try {
                    ...
                    if (result.equals("valid") || result.equals("acceptable")) {
                        if (testResult) {
                            ...
                            failed++;
                            LOG.error("Test {} FAILED: Expected valid/acceptable but encryption/decryption failed", tcId);
                        ...
                        if (!testResult) {
                            ...
                            failed++;
                            LOG.error("Test {} FAILED: Expected invalid but encryption/decryption succeeded", tcId);
                            printTestDetails(test);
                        }
                    }
                } catch (Exception e) {
     ...
     */
    private boolean runCipherTest(String transformation, byte[] key, byte[] iv,
                                   byte[] msg, byte[] ct) {
        try {
